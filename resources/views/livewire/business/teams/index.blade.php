<section class="index-model-section bg-shape-dots">
    <div class="index-model-header">
        ...
        {{-- @livewire('business.teams.teams-table') --}}
        @livewire('business.teams.new-teams-table', ['lazy' => true])
    </div>
    <div class="index-model-actions">
        ...
        <flux:modal.trigger name="{{ $primaryModalName }}">
            <flux:button icon="plus" variant="primary" @click="$dispatch('unable-edit-for-create-team-modal')">
                {{ $primaryBtnLabel }}
            </flux:button>
        </flux:modal.trigger>
        <flux:button variant="ghost">Ir a asesores</flux:button>
    </div>
</section>
